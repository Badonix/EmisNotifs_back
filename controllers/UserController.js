...
const checkGrades = async (req, res) => {
  try {
    ...
    for (const user of users) {
      ...
      const currSubjectMap = new Map();
      sortedCurr.forEach((subj) => {
        currSubjectMap.set(subj.subject, subj.score);
      });

      const newSubjectMap = new Map();
      sortedNew.forEach((subj) => {
        newSubjectMap.set(subj.subject, subj.score);
      });

      for (const [subject, newScore] of newSubjectMap) {
        if (currSubjectMap.has(subject)) {
          const oldScore = currSubjectMap.get(subject);
